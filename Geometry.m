...
% Updated.: 2018-01-11
%%%%%%%------椭圆的短长轴之比固定在0.5
% opt = Option(varargin,'cnt',[0,0],'r',[1,0.5],'q',12,'ang',0);                  % default arguments，r=[1,0.5],1是长半轴的长度，0.5是短半轴的长度
% t = linspace(0,2*pi,opt.q+1)';                                                  % angles 第一个是起始角度，第二个是终止角度，第三个是分割份数
% t = t(1:end-1);
...
%%%%--------------椭圆短长轴之比在0.25~0.75之间随机取值
opt = Option(varargin,'cnt',[0,0],'r',[1,0.5],'q',12,'ang',0);                  % default arguments，r=[1,0.5],1是长半轴的长度，0.5是短半轴的长度
t = linspace(0,2*pi,opt.q+1)';                                                  % angles 第一个是起始角度，第二个是终止角度，第三个是分割份数
t = t(1:end-1);
ratio_ellipse=(0.75-0.25)*rand(1)+0.25;                                         %椭圆的短轴与长轴之比0.25~0.75
out = [opt.cnt(1)+opt.r(1)*cos(t),opt.cnt(2)+(opt.r(1)*ratio_ellipse)*sin(t)];  % ellipse
if opt.ang ~= 0
    out = Rotate(out,'cnt',opt.cnt,'ang',opt.ang);                              % rotated ellipse
end
end
