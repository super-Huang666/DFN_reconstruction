    ...
    if isfield(dfn,'grh')
        ...
        if contains(opt.what,'s')                                               % solution
            if contains(opt.what,'n')                                           % if nodes requested
                ...
                Draw('sph',Stack({g.Node(f).Location}),'r',rt*opt.radius,...    % draws sphere绘制氡浓度
                    'fc',Convert('color',[g.Node(f).Radon],@jet));
%                 Draw('sph',Stack({g.Node(f).Location}),'r',rt*opt.radius,...  % draws sphere绘制压强
%                     'fc',Convert('color',[g.Node(f).Pressure],@jet));
%                   Draw('sph',Stack({g.Node(f).Location}),'r',rt*opt.radius,...   % draws sphere绘制流量
%                     'fc',Convert('color',[g.Edge(f).Flow],@jet));
            end
            if contains(opt.what,'e')                                           % if edges requested
...
%-------------------------输出流量-------------------------------------------------
%                     Draw('cyl',Stack({g.Node([g.Edge(i).Nodes]).Location}),...  % draws cylinder
%                         'r',rt*opt.radius,'cix',int32(Scale([g.Edge(...
%                         g.Edge(i).Nodes).Flow],min([dfn.grh.Edge.Flow]),max([dfn.grh.Edge.Flow]),1,64)));
%----------------------------------------------------------------------------------
%-------------------------输出氡浓度-------------------------------------------------
                    Draw('cyl',Stack({g.Node([g.Edge(i).Nodes]).Location}),...  % draws cylinder
                        'r',rt*opt.radius,'cix',int32(Scale([g.Node(...
                        g.Edge(i).Nodes).Radon],min([g.Node.Radon]),max([g.Node.Radon]),1,64)));
%----------------------------------------------------------------------------------
% %-------------------------输出氡通量-------------------------------------------------
%                     Draw('cyl',Stack({g.Node([g.Edge(i).Nodes]).Location}),...  % draws cylinder
%                         'r',rt*opt.radius,'cix',int32(Scale([g(...
%                         g.Edge(i)).Flux],min([dfn.grh.Flux]'),max([dfn.grh.Flux]'),1,64)));
% %----------------------------------------------------------------------------------
                end
            end
        end
